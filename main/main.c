...
static const char magic[] = "vkfwmark:"
                            "{\"bo\":\"" HW_BOARD "\",\"pr\":\"" PRODUCT "\",\"fv\":" STR(FW_VER) "}";
//

// Default BLE values
static const uint8_t def_srv[] = {0x13, 0x1e, 0x48, 0x11, 0xc2, 0x5c, 0x2c, 0xa6,
                                  0xd2, 0x45, 0x34, 0x81, 0x00, 0x00, 0x00, 0x00};
static const uint8_t def_char[] = {0x13, 0x1e, 0x48, 0x11, 0xc2, 0x5c, 0x2c, 0xa6,
                                   0xd2, 0x45, 0x34, 0x81, 0x01, 0x00, 0x00, 0x00};
...
static void log_add(session_t *s, int32_t op, int32_t par, int32_t res) {
	...
	if (log_elements > 0) { // Check log repetitions
		...
		for (int n = 0; n < log_elements; n++) {
			if (log[f].usr == usr && log[f].op == op && log[f].par == par && log[f].res == res &&
			    (now - log[f].ts) < 60) {
				log[f].rep++;
	...
	log_elements++;
	ESP_LOGI("LOGGER", "[%d] boot:%d cnt:%d usr:%d ts:%d op:%d opd:\"%s\" par:%d res:%d", sh, config.boot_cnt, log_cnt,
	         usr, (int) now, op, code2str(log_ops, op), par, res);
}

static void log_append_msgpack(cw_pack_context *pc) {
	cw_pack_cstr(pc, "lg");
	cw_pack_array_size(pc, log_elements);
	int f = log_front;
	for (int n = 0; n < log_elements; n++) {
...
static void encrypt_response(session_t *s, const char *t, bool seed) {
	...
	cw_pack_cstr(&s->pc_tx, "t");
	cw_pack_cstr(&s->pc_tx, t);
	cw_pack_cstr(&s->pc_tx, "d");
	cw_pack_bin(&s->pc_tx, enc_data, sz + crypto_box_MACBYTES);
	if (seed) {
		cw_pack_cstr(&s->pc_tx, "n");
		cw_pack_bin(&s->pc_tx, s->seed, SEED_SIZE);
	}
	free(enc_data);
	return;
}
...
static int chk_ver_upgrade(session_t *s) {
	...
	if (r) {
		if (r == CW_UNPACK_MAP_ERR_MISSING_KEY)
			ret = 0;
		else
			ret = 2;
...
static int append_egg(session_t *s, cw_pack_context *out) {
	...
	esp_read_mac(mac_bin, ESP_MAC_WIFI_STA);
	sprintf(mac_chr, "%02x%02x%02x%02x%02x%02x", mac_bin[0], mac_bin[1], mac_bin[2], mac_bin[3], mac_bin[4],
	        mac_bin[5]);
	...
	if (ota_next_fv_boot) {
		cw_pack_cstr(&pc, "fv");
		cw_pack_unsigned(&pc, ota_next_fv_boot);
	} else {
		cw_pack_cstr(&pc, "fv");
		cw_pack_unsigned(&pc, FW_VER);
	}
	cw_pack_cstr(&pc, "kv");
	cw_pack_unsigned(&pc, config.key_ver);
	cw_pack_cstr(&pc, "cv");
	cw_pack_unsigned(&pc, config.cfg_ver);
	cw_pack_cstr(&pc, "bo");
	cw_pack_cstr(&pc, HW_BOARD);
	cw_pack_cstr(&pc, "pr");
	cw_pack_cstr(&pc, PRODUCT);
	cw_pack_cstr(&pc, "mf");
	cw_pack_cstr(&pc, MANUFACTURER);
	cw_pack_cstr(&pc, "ma");
	cw_pack_cstr(&pc, mac_chr);
	cw_pack_cstr(&pc, "ts");
	cw_pack_unsigned(&pc, now);
	log_append_msgpack(&pc);
	if (ota.start) {
		cw_pack_cstr(&pc, "st");
		cw_pack_boolean(&pc, ota.start);
		cw_pack_cstr(&pc, "ha");
		cw_pack_bin(&pc, ota.sha256sum, sizeof(ota.sha256sum));
		cw_pack_cstr(&pc, "uv");
		cw_pack_unsigned(&pc, ota.started_update);
		cw_pack_cstr(&pc, "sz");
		cw_pack_unsigned(&pc, ota.size);
		cw_pack_cstr(&pc, "of");
		cw_pack_unsigned(&pc, ota.offset);
	}
...
static void chk_attached_config(session_t *s, uint8_t *buf, size_t buf_len) {
	...
	if (r) {
		...
		for (int i = 0; i < sz; i++) {
			...
			if (array.return_code == CWP_RC_OK &&
			    (array.item.type == CWP_ITEM_NEGATIVE_INTEGER || array.item.type == CWP_ITEM_POSITIVE_INTEGER)) {
...
static int process_egg_frame(session_t *s) {
	...
	if (crypto_box_open_easy_afternm(buf + crypto_box_NONCEBYTES, // Reuse buffer to open server egg
	                                 buf + crypto_box_NONCEBYTES, buf_sz - crypto_box_NONCEBYTES, buf,
	                                 ca_shared) != 0) {
	...
	if (!r) {
		if (tmp_u64 > config.key_ver) {
			if (chk_ver_upgrade(s) == 0) {
				ESP_LOGI("EGG_DOWN", "[%d] Lock upgraded from version:%llu to version:%llu", s->h, config.key_ver,
				         tmp_u64);
				config.key_ver = tmp_u64;
				save_flash_config();
			} else {
				ESP_LOGI("EGG_DOWN", "[%d] Lock upgrade delayed from version:%llu to version:%llu", s->h,
				         config.key_ver, tmp_u64);
			}
		}
	}
...
static int process_login_frame(session_t *s) {
	...
	s->login_len = blob_sz;
	if (crypto_box_open_easy_afternm(s->login_data, buf + crypto_box_NONCEBYTES, buf_sz - crypto_box_NONCEBYTES, buf,
	                                 ca_shared) != 0) {
	...
	if (tmp_u64 > config.key_ver) {
		if (chk_ver_upgrade(s) == 0) {
			...
			ESP_LOGI("LOGIN", "[%d] Lock upgrade delayed from version:%llu to version:%llu", s->h, config.key_ver,
			         tmp_u64);
		}
	}
	...
	cw_pack_cstr(&s->pc_resp, "fv");
	cw_pack_unsigned(&s->pc_resp, FW_VER);
	cw_pack_cstr(&s->pc_resp, "bo");
	cw_pack_cstr(&s->pc_resp, HW_BOARD);
	cw_pack_cstr(&s->pc_resp, "ts");
	cw_pack_unsigned(&s->pc_resp, time(NULL));
...
exitfn:
	...
	if (err > 0) {
		...
		cw_pack_cstr(&s->pc_resp, "e");
		cw_pack_signed(&s->pc_resp, err);
		cw_pack_cstr(&s->pc_resp, "d");
		cw_pack_cstr(&s->pc_resp, code2str(errors, err));
...
static int do_cmd_fs(session_t *s) {
	...
	cw_pack_cstr(&s->pc_resp, "lo");
	cw_pack_boolean(&s->pc_resp, ota_lock);
	cw_pack_cstr(&s->pc_resp, "st");
	cw_pack_boolean(&s->pc_resp, ota.start);
	cw_pack_cstr(&s->pc_resp, "ha");
	cw_pack_bin(&s->pc_resp, ota.sha256sum, sizeof(ota.sha256sum));
	cw_pack_cstr(&s->pc_resp, "bo");
	cw_pack_cstr(&s->pc_resp, HW_BOARD);
	cw_pack_cstr(&s->pc_resp, "uv");
	cw_pack_unsigned(&s->pc_resp, ota.started_update);
	cw_pack_cstr(&s->pc_resp, "rv");
	cw_pack_unsigned(&s->pc_resp, FW_VER);
	cw_pack_cstr(&s->pc_resp, "sz");
	cw_pack_unsigned(&s->pc_resp, ota.size);
	cw_pack_cstr(&s->pc_resp, "of");
	cw_pack_unsigned(&s->pc_resp, ota.offset);
	return 0;
}
...
static int do_cmd_fi(session_t *s) {
	...
	if (ota.start) {
		...
		if (update > ota.started_update || ota.size != size ||
		    memcmp(hash, ota.sha256sum, sizeof(ota.sha256sum)) != 0) {
			reset_ota();
		}
	}
...
exitok:
	...
	err = 0;
	cw_pack_map_size(&s->pc_resp, 1);
	cw_pack_cstr(&s->pc_resp, "r");
	cw_pack_cstr(&s->pc_resp, "ok");
...
static int process_info_frame(session_t *s) {
	...
	cw_pack_cstr(&s->pc_tx, "t");
	cw_pack_cstr(&s->pc_tx, "ri");
	cw_pack_cstr(&s->pc_tx, "pk");
	cw_pack_bin(&s->pc_tx, config.public_key, crypto_box_PUBLICKEYBYTES);
	cw_pack_cstr(&s->pc_tx, "ca");
	cw_pack_bin(&s->pc_tx, ca_key, crypto_box_PUBLICKEYBYTES);
...
exitfn:
	if (ret != 0) {
		...
		if (ret > 0) {
			...
			cw_pack_map_size(&s->pc_tx, 2);
			cw_pack_cstr(&s->pc_tx, "e");
			cw_pack_signed(&s->pc_tx, ret);
			cw_pack_cstr(&s->pc_tx, "d");
			cw_pack_cstr(&s->pc_tx, code2str(errors, ret));
...
static int rx_cb(uint16_t conn, const uint8_t *data, size_t data_len) {
	...
	if (upc.return_code != CWP_RC_OK && upc.return_code != CWP_RC_END_OF_INPUT &&
	    upc.return_code != CWP_RC_BUFFER_UNDERFLOW) {
...
static esp_err_t save_flash_config() {
	...
	cw_pack_cstr(&pc, "fv");
	cw_pack_unsigned(&pc, config.fw_ver);
	cw_pack_cstr(&pc, "kv");
	cw_pack_unsigned(&pc, config.key_ver);
	cw_pack_cstr(&pc, "bc");
	cw_pack_unsigned(&pc, config.boot_cnt);
	cw_pack_cstr(&pc, "cf");
	cw_pack_unsigned(&pc, config.cfg_ver);
	cw_pack_cstr(&pc, "sk");
	cw_pack_bin(&pc, config.secret_key, crypto_box_SECRETKEYBYTES);
	cw_pack_cstr(&pc, "pk");
	cw_pack_bin(&pc, config.public_key, crypto_box_PUBLICKEYBYTES);
	cw_pack_cstr(&pc, "id");
	cw_pack_bin(&pc, config.vk_id, 6);
	cw_pack_cstr(&pc, "tz");
	cw_pack_cstr(&pc, config.tz_data);
	// Application specific config
	cw_pack_cstr(&pc, "$act_tim");
	cw_pack_array_size(&pc, MAX_ACTUATORS);
